...
with tab1 :
    ...
    with co1 :
        col = st.selectbox("select Filter" , color_columns)
        px.histogram(df , x = column , color= col , color_discrete_sequence=px.colors.qualitative.Vivid )
    with co2 :
        col = st.selectbox("select Filter" , color_columns)
        px.scatter(df , x = column , color= col , color_discrete_sequence=px.colors.qualitative.Vivid )
with tab2 :
    st.title("T")
with tab3 :
    st.title("p")
